...
instance
  ...
  {-# INLINEABLE arbitrary #-}
  arbitrary =
    GenValue . Value . AssocMap.safeFromList <$> do
  ...
  shrink (GenValue (Value inner)) = case AssocMap.toList inner of
        ...
        case adaEntry of
          ("", adaInner) -> case AssocMap.toList adaInner of
            [("", adaMod)] -> do
              ...
              let adaEntry' = ("", AssocMap.safeFromList [("", adaMod')])
    ...
    where
      ...
      go kvs = do
        ...
        guard (not . null $ kvs')
        pure . AssocMap.safeFromList $ kvs'
...
mkAdaEntry = do
  ...
  pure ("", AssocMap.safeFromList [("", amount)])
